...
-- Table for API quota tracking (if not exists from previous migration)
...
CREATE INDEX IF NOT EXISTS idx_api_quota_date ON api_quota_usage(api_name, usage_date);
...
BEGIN
  ...
  WHERE c.source = p_source
    ...
    AND c.search_keywords && v_expanded_keywords  -- At least one common keyword
  HAVING (
    SELECT COUNT(*)::DECIMAL / GREATEST(
      array_length(c.search_keywords, 1) + array_length(v_expanded_keywords, 1) -
      (SELECT COUNT(*) FROM unnest(c.search_keywords) k WHERE k = ANY(v_expanded_keywords)),
...
-- Function to calculate dynamic similarity threshold based on query complexity
CREATE OR REPLACE FUNCTION get_dynamic_similarity_threshold(
  p_keywords TEXT[]
)
RETURNS DECIMAL
LANGUAGE plpgsql
...
DECLARE
  v_length INTEGER;
BEGIN
  v_length := COALESCE(array_length(p_keywords, 1), 0);

  -- Shorter queries need higher overlap for confidence
  IF v_length <= 3 THEN
    RETURN 0.7;  -- Short queries: 70% match required
  ELSIF v_length <= 6 THEN
    RETURN 0.6;  -- Medium queries: 60% match required
  ELSE
    RETURN 0.5;  -- Long queries: 50% match required
  END IF;
END;
$$;
