-- Fix Content Search Caching System
-- Corrects issues from the original migration and ensures tables exist

-- Drop the broken function first if it exists
DROP FUNCTION IF EXISTS find_similar_cached_search(TEXT[], TEXT, FLOAT);

-- Recreate tables if they don't exist (safe for re-running)
...
-- Fixed function to find similar cached searches using keyword overlap
-- Corrected: removed invalid HAVING clause without GROUP BY
...
BEGIN
  ...
  WITH keyword_matches AS (
    SELECT
      ...
      ) / GREATEST(array_length(p_keywords, 1), array_length(c.search_keywords, 1), 1) as sim_score
    FROM content_search_cache c
    WHERE c.source = p_source
      AND c.expires_at > now()
      AND c.search_keywords && p_keywords  -- Array overlap operator for initial filtering
  )
  SELECT
    km.id,
    km.search_concept,
    km.results,
    km.sim_score as similarity_score
  FROM keyword_matches km
  WHERE km.sim_score >= p_min_overlap
  ORDER BY km.sim_score DESC
  LIMIT 1;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
...
DROP POLICY IF EXISTS "Service role can manage content cache" ON content_search_cache;
DROP POLICY IF EXISTS "Service role can manage API quota" ON api_quota_tracking;
...
-- Also allow authenticated users to read cache (for debugging/monitoring)
DROP POLICY IF EXISTS "Authenticated users can read cache" ON content_search_cache;
...
DROP POLICY IF EXISTS "Authenticated users can read quota" ON api_quota_tracking;
...
COMMENT ON TABLE content_search_cache IS 'Caches content search results to reduce YouTube API quota usage by 80-96%.';
COMMENT ON TABLE api_quota_tracking IS 'Tracks daily API usage for quota management. YouTube daily limit: 10,000 units.';
COMMENT ON FUNCTION find_similar_cached_search IS 'Finds cached searches with similar keywords. Uses min_overlap (default 60%) for semantic matching.';
