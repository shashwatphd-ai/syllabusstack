  ...
  for (const model of [primaryModel, fallbackModel]) {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        // 402 Payment Required or 429 Rate Limit → fallback to Lovable AI gateway
        if (response.status === 402 || response.status === 429) {
          console.warn(`${logPrefix} OpenRouter ${response.status} — falling back to Lovable AI gateway`);
          const lovableResult = await generateImageLovableAI(request);
          if (lovableResult.success) {
            return lovableResult;
          }
          console.error(`${logPrefix} Lovable AI fallback also failed: ${lovableResult.error.message}`);
        }
...
/**
 * Fallback image generation via Lovable AI gateway (google/gemini-2.5-flash-image)
 * Used when OpenRouter returns 402 (credits exhausted) or 429 (rate limited)
 * @internal
 */
async function generateImageLovableAI(request: {
  prompt: string;
  slideTitle?: string;
  aspectRatio?: '1:1' | '16:9' | '9:16' | '4:3' | '3:4';
  logPrefix?: string;
  ...
  const startTime = Date.now();
  const logPrefix = request.logPrefix || '[Image-LovableAI]';
  ...
  if (!lovableKey) {
    return {
      ...
      error: {
        code: 'CONFIG_ERROR',
        message: 'LOVABLE_API_KEY not configured — cannot use Lovable AI fallback',
        provider: 'openrouter',
        model,
      },
    };
  ...
  console.log(`${logPrefix} Generating image via Lovable AI gateway (${model})`);
  ...
  try {
    ...
    if (!response.ok) {
      ...
      console.error(`${logPrefix} Lovable AI error ${response.status}:`, errText.substring(0, 300));
      return {
        success: false,
        error: {
          code: 'IMAGE_GENERATION_FAILED',
          ...
          model,
          httpStatus: response.status,
          rawError: errText.substring(0, 500),
        },
      };
    }
    ...
    const extracted = extractImageFromResponse(message?.content, message, logPrefix);

    if (!extracted) {
      console.error(`${logPrefix} Could not extract image from Lovable AI response`);
      return {
        success: false,
        error: {
          code: 'INVALID_RESPONSE',
          message: 'Could not extract image from Lovable AI response',
          provider: 'openrouter',
          model,
        },
      };
    ...
    const latency_ms = Date.now() - startTime;
    const imageKB = Math.round(extracted.base64.length / 1024);
    console.log(`${logPrefix} ✓ Image generated via Lovable AI in ${latency_ms}ms (${imageKB}KB)`);
    ...
    console.error(`${logPrefix} Lovable AI exception:`, msg);
    return {
      success: false,
      error: {
        code: 'IMAGE_GENERATION_FAILED',
        message: `Lovable AI fallback failed: ${msg}`,
        provider: 'openrouter',
        model,
        rawError: msg,
      },
